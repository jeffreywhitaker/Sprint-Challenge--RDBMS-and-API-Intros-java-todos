...
public class UserController
{
    ...
    // GET the user and todo based off the authenticated user
    // http://localhost:2019/users/mine
    @PreAuthorize("hasAuthority('ROLE_USER')")
    @GetMapping(value = "/mine", produces = {"application/json"})
    public ResponseEntity<?> getUserAndTodos(Authentication authentication)
    {
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();

        return new ResponseEntity<>(userService.findByName(userDetails.getUsername()), HttpStatus.OK);
    }

    // POST adds a user, can only be done by admin
    ...
    // POST adds a todo to the assigned user. Can be done by any user
    // http://localhost:2019/users/todo/{userid}
    @PreAuthorize("hasAuthority('ROLE_USER')")
    @PostMapping(value = "/todo/{userid}", consumes = {"application/json"}, produces = {"application/json"})
    public ResponseEntity<?> addTodoToAssignedUser(@Valid @RequestBody Todo todo, @PathVariable long userid)
    {
        User postUser = userService.findUserById(userid);
        todo.setUser(postUser);

        return new ResponseEntity<>(todoService.save(todo), HttpStatus.CREATED);
    }

    // DELETE deletes a user based off of their userid and deletes all their associated todos, admin only
    ...
    public ResponseEntity<?> deleteUserById(@PathVariable long userid)
    {
        ...
        todoService.deleteByUser(delUser);
